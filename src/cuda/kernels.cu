...
"""
        ...
        __global__ void calculate_gina_scores(float* impurity_scores,float* X_train,float* y_train,const int unique_classes,const int l,const int w){
            int Dim = threadIdx.x+blockIdx.x*blockDim.x;
            int Row = threadIdx.y+blockIdx.y*blockDim.y;
            if(Dim < w && Row < l){
                float split_value =X_train[Row * w+ Dim];

                int group1_counts[20];//Max of 20 dimensions which can be increased
                int group2_counts [20];
                int length1=0;
                int length2=0;
                int sum1=0;
                int sum2=0;
