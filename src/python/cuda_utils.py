...
class DecisionTreeCudaUtils():
    ...
    def calculate_score(self, X_train_b, y_train_b):
        """Calculate gini impurity for all X datapoints

        Args:
            X_train_b (np.array): The X training data with any number of rows and columns
            y_train_b (np.array): The y categorical labeled data

        Raises:
            Exception: If X is not the correct data type
            Exception: If y is not the correct data type

        Returns:
            tuple(np.array, list(TimingObjects))
                np.array is the gini impurity scores with the same shape as X_train_b
                list(TimingObjects)) contain useful timing information
        """
    ...
    def split_data(self, X: np.array, y: np.array, bound: float, dim: float):
        """Split X and y arrays into two groups based on dimension and the boundary

        Args:
            X (np.array): 
            y (np.array): 
            bound (float): The condition to split the data using
            dim (int): 
    ...
    def choose_best_score(self,all_gina_scores: np.array):
        ...
        Args:
            all_gina_scores (np.array): The array of scores

        Raises:
            Exception: If the passed array is of the wrong type

        Returns:
            tuple(): max index and a list of timing objects
        """
